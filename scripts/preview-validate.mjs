...
// Coverage is judged against the recipes this archive actually uses (a catalogue run may use two).
const expected = new Set(controlled.map(record => record.recipeId))
const partial = [...recipesPerFace].filter(([, set]) => set.size < expected.size)
...
say(`faces with all ${expected.size} recipes`, `${recipesPerFace.size - partial.length} of ${recipesPerFace.size}`)
