...
try {
  ...
  for (const [face, indexes] of byFace) {
    ...
    const images = await page.evaluate(async ({ base64, lines }) => {
      const font = new FontFace('reference', Uint8Array.from(atob(base64), c => c.charCodeAt(0)))
      await font.load(); document.fonts.add(font)
      try {
        return lines.map(text => {
          const c = document.createElement('canvas'), ctx = c.getContext('2d'); ctx.font = '48px reference'
          ...
          ctx.fillStyle = 'white'; ctx.fillRect(0, 0, c.width, c.height); ctx.fillStyle = 'black'; ctx.font = '48px reference'
    ...
    }, { base64: bytes.toString('base64'), lines: indexes.map(i => references.samples[i].text) })
    for (const [n, i] of indexes.entries()) {
      const { width, height } = images[n], gray = Buffer.from(images[n].pixels, 'base64'), data = new Uint8Array(width * height * 4)
      for (let p = 0; p < gray.length; p++) { data.fill(gray[p], p * 4, p * 4 + 3); data[p * 4 + 3] = 255 }
      prepared[i] = prepareLine({ width, height, data }, { deskew: true, sampler: 'windows' }).windows
    }
    if (++done % 500 === 0) console.log('references', done, '/', byFace.size)
  }
  // Keep the Pillow manifest's sample order; windows follow it.
  const samples = []
  for (const [i, items] of prepared.entries()) {
    if (!items?.length) continue
    const source = samples.length; samples.push(references.samples[i])
    for (const w of items) {
      const raw = Buffer.from(Uint8Array.from(w.pixels, v => Math.round(v * 255)))
      windows.push({ source, offset, width: w.width, height: w.height }); await file.write(raw); offset += raw.length
    }
  }
  await file.close()
  const pins = Object.fromEntries(await Promise.all(['scripts/style-references-browser.mjs', 'src/line.mjs', 'src/input.mjs', 'src/prepare.mjs'].map(async p => [p, hash(await readFile(p))])))
  await writeFile('.data/style/references-browser.json', JSON.stringify({ facesSha256: references.facesSha256, pins, browser: browser.version(),
    sha256: hash(await readFile('.data/style/references-browser.u8')), samples, windows }) + '\n')
