...
export function sourceCatalogs(inputs, binding) {
  ...
  for (const { catalog, records } of inputs) {
    ...
    const byId = new Map(records.map(r => [r.id, r])), raw = Buffer.from(catalog.vectors.data, 'base64'), size = rowBytes(catalog.vectors), { encoding } = catalog.vectors
  ...
  return [...groups].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([id, group]) => {
    ...
    const data = { version: 3, kind: 'font-catalog', ...binding, dimensions: 128, source: id, sourceManifests: [...group.manifests].sort(), faces,
      vectors: { encoding: [...encodings][0], shape: [owners.length, 128], data: Buffer.concat(chunks).toString('base64'), scales, owners } }
