import { readCatalog, rowBytes } from '../src/catalog.mjs'
...
export function sourceCatalogs(inputs, binding) {
  ...
  for (const { catalog, records } of inputs) {
    ...
    const byId = new Map(records.map(r => [r.id, r])), raw = Buffer.from(catalog.vectors.data, 'base64'), size = rowBytes(catalog.vectors, catalog.dimensions), { encoding } = catalog.vectors
    for (const [owner, face] of catalog.faces.entries()) {
      ...
      if (!groups.has(source.key)) groups.set(source.key, { name: names[source.key] || source.name, faces: new Map(), manifests: new Set(), dimensions: catalog.dimensions })
      if (groups.get(source.key).dimensions !== catalog.dimensions) throw new Error('Source catalogs of different embedding dimensions')
      const group = groups.get(source.key), rows = catalog.vectors.owners.flatMap((o, i) => o === owner ? [i] : [])
      const entry = { face, encoding, data: Buffer.concat(rows.map(i => raw.subarray(i * size, (i + 1) * size))).toString('base64'), scales: rows.map(i => catalog.vectors.scales[i]) }
  ...
  return [...groups].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([id, group]) => {
    const faces = [], chunks = [], scales = [], owners = [], encodings = new Set([...group.faces.values()].map(entry => entry.encoding))
    if (encodings.size !== 1) throw new Error('Source catalogs pack their vectors differently')
    ...
    const data = { version: 3, kind: 'font-catalog', ...binding, dimensions: group.dimensions, source: id, sourceManifests: [...group.manifests].sort(), faces,
      vectors: { encoding: [...encodings][0], shape: [owners.length, group.dimensions], data: Buffer.concat(chunks).toString('base64'), scales, owners } }
