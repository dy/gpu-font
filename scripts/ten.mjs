...
async function render(config) {
  python('train.ten_data', 'plans')
  const plans = (await read(`${dir}/plans.json`)).filter(p => p.role !== 'train' || p.index < config.browserTrain)
  const fonts = (await read('bench/fonts.json')).fonts
  ...
  try {
    ...
    for (const family of config.fonts) {
      const face = fonts.find(f => f.id === family), bytes = await readFile(`.data/fonts/${family}.ttf`)
  ...
  try {
    for (const [id, sample] of source.samples.entries()) {
      ...
      if (prepared.status !== 'ok') throw new Error(`Empty text: ${id}`)
  ...
  await write(`${dir}/prepared.json`, { ...source, windows, tensorSha256: tensorHash.digest('hex'),
    ...
    runnerSha256: hash(await readFile('scripts/ten.mjs')) })
...
const config = await read('bench/ten.json')
await mkdir(dir, { recursive: true })
if (['all', 'data'].includes(command)) await render(config.data)
if (['all', 'train'].includes(command)) python('train.ten', 'train')
if (command === 'evaluate') python('train.ten', 'evaluate')
