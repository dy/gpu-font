...
import { embedWindows, unit } from './catalog.mjs'
...
// One reference line, black on white, as the reference renders store it: the red channel as gray.
export function drawLine(ctx, family, text) {
...
// Rows as the repository's exporter packs them: L2-normalized, int8 with one scale per row, scales to 6 significant digits.
export function packVectors(rows) {
  const scales = rows.map(r => Math.max(Math.max(...r.map(Math.abs)) / 127, 1e-12)), packed = new Int8Array(rows.length * 128)
  rows.forEach((r, i) => r.forEach((v, d) => { packed[i * 128 + d] = Math.max(-127, Math.min(127, Math.round(v / scales[i]))) }))
  return { encoding: 'int8-base64', shape: [rows.length, 128], data: toBase64(new Uint8Array(packed.buffer)), scales: scales.map(s => Number(s.toPrecision(6))) }
}
...
// Both catalogs' faces and rows in one; a face of the second replaces a face of the first with the same id.
export function joinCatalogs(first, second) {
  const replaced = new Set(second.faces.map(f => f.id)), faces = [], rows = [], scales = [], owners = []
  for (const [n, catalog] of [first, second].entries()) {
    ...
    catalog.vectors.owners.forEach((owner, r) => {
      ...
      rows.push(bytes.subarray(r * 128, r * 128 + 128)); scales.push(catalog.vectors.scales[r]); owners.push(index.get(owner))
    })
  }
  const packed = new Uint8Array(rows.length * 128)
  rows.forEach((row, i) => packed.set(row, i * 128))
...
export async function buildCatalog(faces, { project, preparation, binding, source = 'my-fonts', progress = () => {}, signal }) {
  ...
  // One vector per case: each line's windows embedded as a crop's are, the lines averaged.
  ...
  for (const [done, face] of faces.entries()) {
    ...
    try {
      cases = Object.values(plan(ctx, font.family))
        .map(lines => lines.map(text => prepareLine(drawLine(ctx, font.family, text), { ...preparation, deskew: true, sampler: 'windows' })).filter(p => p.status === 'ok').map(p => p.windows))
  ...
  const catalog = entries.length ? { version: 3, kind: 'font-catalog', ...binding, dimensions: 128, source, referenceMethod: 'faces-cases-browser',
