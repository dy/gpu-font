...
try {
  ...
  assert.equal(await page.locator('.results-column > .column-head > #matches-title + #catalog-control > #catalog-button').count(), 1)
  assert.equal(await page.locator('.results-column > .column-head + .input-meta > #detection-time').count(), 1)
  ...
  assert.deepEqual(await page.locator('#model-input .panel-head > :first-child, #catalog-control > :first-child').allTextContents(), ['Model input', 'Catalog'])
