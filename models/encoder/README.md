...
- `catalogs/`: one catalog per free source in `bench/open-fonts.json` (names, links, vectors; no fonts), built by `scripts/catalog-files.mjs` from the pinned files: every face, one reference per Latin case, drawn by Chromium and embedded by this int8 encoder. Only sources whose terms `bench/foundries.json` records as `permitted` or `none-found` are written here; others stay in `.data/catalogs/withheld/`. Format: version 3.
