...
This is the selected artifact from the [frozen shared-encoder experiment](../../bench/encoder.md). It is not the model deployed in the demo, and it has not met the retrieval or rejection gates.
...
Reproduce indexing and evaluation with `node scripts/python.mjs -m train.encoder_catalog test` after preparing the pinned final dataset; see the experiment report for all data/training commands. Verify CPU/Metal parity with `node checks/encoder.mjs`. The public JavaScript catalog-loading/search API is not yet released; the browser check uses the existing convolution kernels through a diagnostic adapter.
