...
Uniform rescaling already exists. It cannot recover serifs erased at the original raster size. Deskewing a long rotated line may improve useful glyph resolution because the current bounding box height includes the line's vertical displacement. This is a hypothesis to measure, not a result. Added plain background alone already leaves inference pixels unchanged; other marks can change the foreground bounds.
...
The immediate next experiment is step 1–2: complete-glyph grouping and deskew, with the model frozen. The large-catalog candidate is a **shared neural style encoder plus compact reference vectors**, retaining a classifier baseline. Its benefits must be measured; it is not the archived handcrafted descriptor path.
