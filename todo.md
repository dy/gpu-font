...
Current local demo: **shared encoder with interchangeable catalogs**, defaulting to 2,004 Google Fonts families. Two raster-preview archives can be compiled into separate phrase, alphabet and source-page catalogs. The historical 100-family classifier remains reproducible with `npm run demo:build -- --hundred`. Encoder accuracy and rejection remain below the release gates; this is an experimental integration, not a quality promotion.
...
Shared-encoder result: the [first matched comparison](bench/encoder.md) is complete. The selected 489 KB encoder indexes all 2,004 eligible families without retraining; encoder/catalog/existing runtime components total 1.43 MB. Final unseen-family macro top-5 is 54.75% and rejection coverage is 0.0277%, so the integrated demo remains experimental.
...
- [ ] First improve reference generation with the frozen encoder: compare script-separated or clustered prototypes against the current cross-script mean, under the same byte budget and using development queries only. The selected encoder's development macro top-5 is 72.24% for single-script families versus 36.55% for multi-script families; this is a diagnostic association, not proof of the cause. Reserve new final queries before another selection.
