...
- [x] Compare one versus four references per selected genuine face at equal source-image budgets; one wins this experiment. Preserve actual weight/style metadata without claiming to predict it. Int8 weight/vector loss is 0.073 percentage points; [encoder/catalog/current runtime components](bench/encoder-runtime.json) total 1,425,621 bytes. The complete released package is still gated below.
