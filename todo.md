...
Style-encoder result: the [style encoder](bench/style.md) (1.36M parameters, int8, typed heads) with a per-face Chromium-reference catalog puts the right family in the top five for 84.3% of 5–10 character crops from 300 held-out families (87.8% at 8–10 characters, 62.5% top-1, identical designs counted), against 60.5% for the previous encoder on the same frozen benchmark. All eight reported demo samples rank in the top five, seven first. Model/catalog/required modules total 7.9 MB. Hand drawings, other rasterizers, real screenshots and rejection remain below release readiness.

- [x] Collect and audit two raster-preview batches: 309 reference records, 48 families and 56 genuine faces. Preserve original pixels, verified metadata and regions independently of encoder versions. See [collection/catalog audit](bench/preview-catalogs.md).
- [ ] Compare alphabet, phrase and source-page reference recipes using separately collected query screenshots before broad collection. Catalog references remain separate from evaluation queries.
...
- [x] Implement raster-preview → frozen encoder → versioned catalog compilation and a demo selector/import. Preserve genuine faces, exact encoder/preparation bindings and candidate-only ranking. See [preview catalogs](bench/preview-catalogs.md). WebGPU computes embeddings; exact cosine ranking currently runs in JavaScript.
- [ ] Measure independent screenshot queries and meet the published quality gates before releasing the catalog API as reliable recognition.
...
- [x] Preview each match in its own face (the static instance its references were rendered from), compile collected catalogs with one reference per capture, and measure catalog swap: 100% top-five in each source's catalog, 97.1% among Google's families. See [catalog swap](bench/preview-catalogs.md#catalog-swap).
- [ ] Collect script, brush, handwriting and contemporary serif faces for the Adobe Fonts and MyFonts catalogs; the archive has none, and those styles score well among Google families (brush 96.7%, display 90.4% held-out top-five), so the gap is coverage.
- [ ] Index a user's own font files into a catalog in one command (Chromium references, as for Google Fonts); today a catalog comes from the Google pipeline or a preview archive.
...
- [ ] Train style to cross case and script: held-out capitals found among lowercase-only references 46.9% of the time, other scripts from Latin only 42.0%, Chinese 19.2% ([breakdown](bench/style.md#by-requirement)); collected catalogs often hold one case.
- [ ] Raise non-Latin scripts (64%) and Hanzi (49%) toward Latin, and seen families at 8–10 characters from 87.3% toward the 95% target.
